...
const FrontDeskQueue = () => {
  ...
  const fetchQueue = async () => {
    try {
      const res = await api.get(`/frontdesk/queue/active?facilityId=${user?.facilityId}`, {
  ...
  const handleCallNext = async () => {
    ...
    try {
      const res = await api.post('/frontdesk/queue/call-next', {
        doctorId: '00000000-0000-0000-0000-000000000000', // Assign default doctor context for clinic walk-in test verification
  ...
  return (
    <NavigationShell role="frontdesk" userName={user?.name} onLogout={logout}>
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
          ...
          <div className="flex gap-3">
            <Button 
              variant="secondary" 
              onClick={() => triggerToast('Direct check-ins can be managed via the Calendar scheduling view', 'info')}
            >
              Check In Patient
            </Button>
            <Button 
