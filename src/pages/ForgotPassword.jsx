...
import { Mail, ArrowRight, ArrowLeft, Activity } from 'lucide-react';
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setToastMessage('OTP code has been sent successfully to your email.');
        setTimeout(() => {
      ...
      setToastType('error');
      setToastMessage(err.error || 'Failed to request verification code.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      <div className="flex flex-col items-center w-full max-w-[480px] z-10">
        ...
        <div className="bg-white rounded-[24px] border border-[#D8E7E5] shadow-[0_20px_60px_rgba(13,72,70,0.12)] p-8 md:p-10 w-full animate-float-card">
          <h2 className="text-2xl font-bold font-heading text-[#082F2D] text-center mb-1">Forgot Password</h2>
          <p className="font-sans text-sm text-[#3F5E5C] text-center mb-8">
            Enter your registered email address. We will send a verification code.
          </p>
          ...
          <div className="text-center font-sans text-sm text-[#3F5E5C]">
            <a 
              href="/login" 
              className="text-[#0D4846] font-semibold hover:underline inline-flex items-center gap-1 hover:text-[#11615D] transition-colors"
            >
